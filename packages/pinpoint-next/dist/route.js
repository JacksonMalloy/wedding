...
var StatusSchema = z.enum(["pending", "fixed", "wontfix", "deferred"]);
...
async function spawnAgent(ctx) {
  ...
  const logsDir = join2(ctx.projectRoot, ".pinpoint", "logs");
  await mkdir2(logsDir, { recursive: true });
  const logPath = join2(logsDir, `${ctx.feedback.id}.md`);
...
async function createWorktree(projectRoot, feedbackId, logPath) {
  if (!existsSync2(join2(projectRoot, ".git"))) {
    throw new Error("project root is not a git repository");
  }
  const worktreeDir = join2(projectRoot, ".pinpoint", "worktrees");
  await mkdir2(worktreeDir, { recursive: true });
  const worktreePath = join2(worktreeDir, feedbackId);
...
async function openInEditor(projectRoot, file, line, col) {
  ...
  const abs = isAbsolute(file) ? file : resolve2(projectRoot, file);
  const rootAbs = resolve2(projectRoot);
  if (!abs.startsWith(rootAbs + sep) && abs !== rootAbs) {
    throw new Error("path outside project root");
  }
  if (!existsSync3(abs)) throw new Error(`file not found: ${file}`);
...
async function POST(req, ctx) {
  ...
  const decoded = Buffer2.from(parsed.data.screenshot, "base64");
...
async function readJsonBody(req) {
  ...
  const text = Buffer2.from(ab).toString("utf8");
